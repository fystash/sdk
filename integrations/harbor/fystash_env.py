...
# The control plane rejects any argv entry over 4096 bytes. Leave headroom for
# the docker-exec wrapper and shell quoting.
_MAX_EXEC_ARGV_BYTES = 3500
# Reads the real command off stdin into a temp file so argv stays small.
_STDIN_BOOTSTRAP = 'f=$(mktemp); cat >"$f"; {run}; rc=$?; rm -f "$f"; exit $rc'
...
class FystashEnvironment(BaseEnvironment):
    ...
    async def _prepare_task_container_apt(self) -> None:
        ...
        r = await self._host_exec(
            f"docker exec -u root {shlex.quote(self._ctr)} bash -lc "
    ...
    async def exec(
        ...
        if not self._dind:
            ...
            if len(command.encode()) <= _MAX_EXEC_ARGV_BYTES:
                return await self._host_exec(
                    command, timeout_sec=timeout, env=merged, cwd=work
                )
            return await self._host_exec(
                _STDIN_BOOTSTRAP.format(run='bash -l "$f"'),
                timeout_sec=timeout,
                env=merged,
                cwd=work,
        ...
        if len(wrapped.encode()) <= _MAX_EXEC_ARGV_BYTES:
            return await self._host_exec(wrapped, timeout_sec=timeout)
        # Agents that write a whole file in one heredoc overflow the argv
        # ceiling; send the script over stdin and keep argv short instead.
        run = (
            f"docker exec -i{env_flags}{user_flag} -w {shlex.quote(work)} "
            f'{shlex.quote(self._ctr)} bash -l -s <"$f"'
        )
        return await self._host_exec(
            _STDIN_BOOTSTRAP.format(run=run),
            timeout_sec=timeout,
            stdin=command.encode(),
        )
    ...
    async def _write_bytes(self, target_path: str, data: bytes) -> None:
        ...
        if self._dind:
            script = (
                f"docker exec -i -u root {shlex.quote(self._ctr)} bash -lc "
                f"{shlex.quote(inner)}"
            )
        else:
            script = inner
    ...
    async def upload_dir(self, source_dir: Path | str, target_dir: str) -> None:
        ...
        for attempt in range(1, 4):
            try:
                ...
                if self._dind:
                    # File transfer runs as root, like the `docker cp` the
                    # reference environment uses. Without it an image whose
                    # default USER is unprivileged cannot receive /solution or
                    # /tests, which land at the filesystem root.
                    extract_inner = (
